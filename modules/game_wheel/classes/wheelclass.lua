...
local function getLoadedPlayer()
  local player = LoadedPlayer or g_game.getLocalPlayer()
  if player and type(player.isLoaded) == "function" then
    return player
  end
  return nil
end

if not table.isIn then
  function table.isIn(t, val)
...
function WheelOfDestiny.canAddPoints(index, ignoreMaxPoint)
  ...
  if not WheelOfDestiny.scrollPoints then
    WheelOfDestiny.scrollPoints = 0
  end

  if not WheelOfDestiny.extraGemPoints then
    WheelOfDestiny.extraGemPoints = 0
  end
...
function WheelOfDestiny.onDestinyWheel(playerId, canView, changeState, vocationId, points, scrollPoints, pointInvested, usedPromotionScrolls, equipedGems, atelierGems, basicUpgraded, supremeUpgraded, earnedFromAchievements)
  -- Se vocationId for inválido, tenta obter do jogador local
  if not table.isIn({1, 2, 3, 4, 5}, vocationId) then
    local player = g_game.getLocalPlayer()
    if player then
      local serverVocation = player:getVocation()
      vocationId = translateWheelVocation(serverVocation)
    end
  end
  
  -- Verifica novamente após tentar obter do jogador
  if not table.isIn({1, 2, 3, 4, 5}, vocationId) then
...
function WheelOfDestiny.create(playerId, canView, changeState, vocationId, points, scrollPoints, pointInvested, usedPromotionScrolls, equipedGems, atelierGems, basicUpgraded, supremeUpgraded, earnedFromAchievements)
  ...
  WheelOfDestiny.points = points or 0
  WheelOfDestiny.levelPoints = points or 0
  WheelOfDestiny.scrollPoints = scrollPoints or 0
  WheelOfDestiny.usedPromotionScrolls = usedPromotionScrolls or {}
  WheelOfDestiny.equipedGems = equipedGems or {}
  WheelOfDestiny.atelierGems = atelierGems or {}
  WheelOfDestiny.basicModsUpgrade = basicUpgraded or {}
  WheelOfDestiny.supremeModsUpgrade = supremeUpgraded or {}
  WheelOfDestiny.extraGemPoints = 0
  WheelOfDestiny.fromAchievementType = earnedFromAchievements or 0
  ...
  if WheelOfDestiny.basicModsUpgrade then
  ...
  if WheelOfDestiny.supremeModsUpgrade then
...
function onWheelOfDestinyApply(close, ignoreprotocol)
    ...
    if not ignoreprotocol then
        ...
        g_game.sendApplyWheelPoints(
            WheelOfDestiny.pointInvested,
            struct[GemDomains.GREEN].gemID,
            struct[GemDomains.RED].gemID,
            struct[GemDomains.ACQUA].gemID,
            struct[GemDomains.PURPLE].gemID
        )
    end
...
local inactiveColor = "#AAAAAA" -- 

if passive == 1000 then
    wheelOfDestinyWindow.selection.tabContent.dedication:setColor(activeColor)
else
    wheelOfDestinyWindow.selection.tabContent.dedication:setColor(inactiveColor)
end


  wheelOfDestinyWindow.selection.tabContent.conviction:setText("Locked")
  wheelOfDestinyWindow.selection.tabContent.conviction:setColor(inactiveColor)

local activeColor = "#FFFFFF"    -- 
local inactiveColor = "#AAAAAA"  -- 
if passive >= 1000 then
    wheelOfDestinyWindow.selection.tabContent.conviction:setText("Stage 3")
    wheelOfDestinyWindow.selection.tabContent.conviction:setColor(activeColor)
elseif passive >= 500 then
...
function WheelOfDestiny.getExportCode(preset)
  ...
  local base64Data = base64.encode(data)
  local lp = getLoadedPlayer()
  if not lp or type(lp.getVocation) ~= "function" then
    return ""
  end
  local currentVocation = translateWheelVocation(lp:getVocation())
...
function WheelOfDestiny.loadWheelPresets()
	...
	local lp = getLoadedPlayer()
	if not lp or type(lp.isLoaded) ~= "function" or not lp:isLoaded() then
		return true
	end

	local defaultData = { presets = {{ exportString = defaultExportString[translateWheelVocation(lp:getVocation())], name = "Default-Preset" }}} -- default data

	local file = "/characterdata/" .. lp:getId() .. "/wheelOfDestiny.json"
...
function WheelOfDestiny.saveWheelPresets()
  ...
  for _, preset in pairs(WheelOfDestiny.internalPreset) do
    ...
    if not string.empty(exportStr) and #exportStr < 10 then
      if type(lp.getVocation) == "function" then
        exportStr = defaultExportString[translateWheelVocation(lp:getVocation())]
      end
    end
  ...
  local file = "/characterdata/" .. (type(lp.getId) == "function" and lp:getId() or "0") .. "/wheelOfDestiny.json"
