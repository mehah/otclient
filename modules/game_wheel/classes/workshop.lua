...
-- Enhancement costs for mods (based on server getLesserGradeCost and getGreaterGradeCost)
lesserResources = {
	[0] = {price = 2000000, fragment = 5},  -- Grade 1: 2M gold, 5 fragments
	[1] = {price = 5000000, fragment = 15}, -- Grade 2: 5M gold, 15 fragments
	[2] = {price = 30000000, fragment = 30} -- Grade 3: 30M gold, 30 fragments
}

greaterResources = {
	[0] = {price = 5000000, fragment = 5},  -- Grade 1: 5M gold, 5 fragments
	[1] = {price = 12000000, fragment = 15}, -- Grade 2: 12M gold, 15 fragments
	[2] = {price = 75000000, fragment = 30} -- Grade 3: 75M gold, 30 fragments
}
...
function Workshop.createFragments()
	...
	local wheelVocationId = translateWheelVocation(player:getVocation())
	
	-- Map translateWheelVocation result to VocationSupremeMods keys
	local vocationMap = {
		[1] = 8,  -- Elite Knight -> VocationSupremeMods[8]
		[2] = 7,  -- Royal Paladin -> VocationSupremeMods[7]
		[3] = 5,  -- Master Sorcerer -> VocationSupremeMods[5]
		[4] = 6,  -- Elder Druid -> VocationSupremeMods[6]
		[5] = 9   -- Elite Monk -> VocationSupremeMods[9]
	}
	local vocationId = vocationMap[wheelVocationId] or 0
...
function Workshop.onFragmentClick(widget)
    -- Handle direct click on fragment widget
    if widget and widget.cache then
        -- Focus the widget to update UI
        local fragmentPanel = fragmentWindow:recursiveGetChildById('fragmentContent')
        if fragmentPanel then
            fragmentPanel:focusChild(widget)
        end
    end
    return true
end
...
function Workshop.onSelectChild(list, selected)
    ...
    local activeColor = "#dfdfdf"
    local inactiveColor = "#888888"
    ...
    for i = 0, 3 do
		...
		if isSupreme then
			...
			-- supremeWidget:setImageShader(isActive and '' or 'image_black_white') -- Method not available in this client
		else
			...
			-- basicWidget:setImageShader(isActive and '' or 'image_black_white') -- Method not available in this client
		end

        -- backDrop:setImageShader(isActive and '' or 'image_black_white') -- Method not available in this client
    ...
    -- Get the NEXT tier cost (what we're upgrading TO, not FROM)
    local currentTier = isSupreme and supremeTier or basicTier
    local nextTier = currentTier + 1
    
    -- Check if we can upgrade further (max tier is 3)
    if nextTier > 3 then
    ...
    local goldCost = isSupreme and greaterResources[nextTier - 1].price or lesserResources[nextTier - 1].price
    local fragmentCost = isSupreme and greaterResources[nextTier - 1].fragment or lesserResources[nextTier - 1].fragment
    local resourceCheck = isSupreme and (player:getResourceBalance(ResourceTypes.GREATER_FRAGMENT) or 0) or (player:getResourceBalance(ResourceTypes.LESSER_FRAGMENT) or 0)
...
function Workshop.onUpgradeModification(button)
	...
	-- fragmentType: 0 = Greater, 1 = Lesser
	local fragmentType = selectedWidget.cache.supreme and 0 or 1
	local pos = selectedWidget.cache.modID
	
	g_game.sendGemAtelierAction(4, fragmentType, pos, false)
end
