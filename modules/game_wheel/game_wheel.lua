...
function isVisible()
    ...
    return controllerWheel.ui:isVisible()
end
...
function toggle()  
    ...
    -- Usa o MainWindowManager para garantir que apenas uma janela principal esteja aberta
    if MainWindowManager then
        if isVisible() then
            return hide()
        end
        return MainWindowManager.show("wheel", "wheelOfDestiny")
    end
    
    -- Fallback: Se passou em todas as verificações, abrir o módulo  
...
function controllerWheel:onGameStart()  
    if g_game.getClientVersion() >= 1310 then  
        ...
        wheelButton = modules.game_mainpanel.addToggleButton('WheelButton', tr('Wheel of Destiny'),  
            '/images/options/wheel', function() toggle() end, false, 8, true)  
        local btn = wheelButton:getChildById('button')
        if btn then btn:setOn(false) end
        
        -- Connect to resource balance event for wheel points highlight
        connect(g_game, { onResourcesBalanceChange = onWheelResourceBalance })  
...
-- Store wheel points for highlight logic
Wheel.availablePoints = 0

function controllerWheel:onGameEnd()  
    hide()
    disconnect(g_game, { onResourcesBalanceChange = onWheelResourceBalance })
end  
...
-- Handle resource balance updates for wheel highlight
-- Parameters: value, oldBalance, resourceType (from onResourcesBalanceChange)
function onWheelResourceBalance(value, oldBalance, resourceType)
    if resourceType == ResourceWheelPoints then
        Wheel.availablePoints = value
        updateWheelHighlight()
    end
end

-- Update wheel button highlight based on available points
function updateWheelHighlight()
    if wheelButton then
        ...
        local shouldShow = Wheel.availablePoints > 0
...
function hide()  
    ...
    -- Re-show highlight if there are still wheel points available
    updateWheelHighlight()
end
...
-- Função interna para mostrar o Wheel (chamada pelo MainWindowManager)
local function showInternal(defaultWindow)  
...
function show(defaultWindow)
    -- Usa o MainWindowManager para fechar outras janelas antes de abrir
    if MainWindowManager then
        MainWindowManager.show("wheel", defaultWindow)
    else
        showInternal(defaultWindow)
    end
end

-- Registra o Wheel no MainWindowManager
-- Chamado diretamente sem scheduleEvent para evitar problemas de função expirada
if MainWindowManager then
    MainWindowManager.register("wheel", showInternal, hide, isVisible)
end
