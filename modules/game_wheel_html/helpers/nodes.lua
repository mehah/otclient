...
-- 3 - 100
WheelNodes[19].connecteds = { 13, 20, 26, 25, 32 }
...
local function canReachConnectedNodes(node, ignoreNode)
    ...
    while #nodesToVisit > 0 do
        ...
        for _, connectedNode in ipairs(currentNode.connecteds) do
            if not visitedNodes[connectedNode] and WheelController.wheel:isSlotFull(connectedNode) and connectedNode ~= ignoreNode then
...
local nodes = {
    canReachConnectedNodes = canReachConnectedNodes,
