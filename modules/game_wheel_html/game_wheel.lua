...
WheelController.availablePoints = 0
...
function updateWheelHtmlHighlight()
    if WheelButton then
        local highlight = WheelButton:getChildById('highlight')
        local bright = WheelButton:getChildById('brightButton')
        local shouldShow = WheelController.availablePoints > 0
...
function onResourcesBalanceChange(balance, oldBalance, type)
    ...
    -- Also handle wheel highlight for wheel points
    if type == ResourceWheelPoints then
        WheelController.availablePoints = balance
        updateWheelHtmlHighlight()
    end
end
