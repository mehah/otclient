...
function ImbuementItem.onSelectSlotClear(imbuement)
    ...
    if clearButton then
        ...
        clearButton.onClick = function()
            ...
            self.confirmWindow = displayGeneralBox(tr('Confirm Clearing'),
                tr("Do you wish to spend %s gold coins to clear the imbuement \"%s\" from your item?",
                    comma_value(imbuement[3]), string.capitalize(imbuement[1].name)),
        ...
        clearButton.onHoverChange = function(widget, hovered, itemName, hasItem)
            if hovered then
                cleanImbuementsDetails:setText(tr(
...
function ImbuementItem.selectBaseType(selectedButtonId)
    ...
    for id, imbuement in pairs(self.availableImbuements) do
        if imbuement.tier == baseImbuement then
...
function ImbuementItem.onSelectImbuement(widget)
    ...
    local itemDetails = self.window:recursiveGetChildById("itemDetails")
    if itemDetails then
        itemDetails:setText("")
    end
end
...
function ImbuementItem.selectImbuementWidget(widget, imbuement)
                ...
                if source then
                    ...
                    itemWidget.onHoverChange = function(widget, hovered)
                        ...
                        if hovered then
                            local itemCount = self.needItems[source.item:getId()] or 0
                            if itemCount >= source.item:getCount() then
                                itemDetails:setText(string.format("The imbuement you have selected requires %s.",
                                    source.description))
                            else
                                itemDetails:setText(string.format(
                                    "The imbuement requires %s. Unfortunately you do not own the needed amount.",
                                    source.description))
                            end
                        else
                            if itemDetails then
    ...
    if imbueApply then
        ...
        imbueApply.onHoverChange = function(widget, hovered, itemName, hasItem)
            local itemDetails = self.window:recursiveGetChildById("itemDetails")
            if hovered then
                itemDetails:setText(tr(
                    "Apply the selected imbuement. This will consume the required astral sources and gold."))
            else
                if itemDetails then
                    itemDetails:setText("")
                end
        ...
        imbueApply.onClick = function()
            ...
            self.confirmWindow = displayGeneralBox(tr('Confirm Imbuing'),
                tr(
                    "You are about to imbue your item with \"%s\". This will consume the required astral sources and %s\ngold coins. Do you wish to proceed?",
                    string.capitalize(imbuement.name), comma_value(imbuement.cost)),
                { { text = tr('Yes'), callback = confirm },
                    { text = tr('No'),  callback = cancelFunc },
                }, confirm, cancelFunc)
        end
