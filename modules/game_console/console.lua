...
--font otfont or ttf
consoleFont = 'verdana-bold'
--only TrueTypefont
consoleFontSize = 11

function setConsoleFont(font, size)
    consoleFont = font
    consoleFontSize = size or 0
end
...
function addTabText(text, speaktype, tab, creatureName)
    ...
    if consoleFontSize > 0 then
        label:setTTFFont(consoleFont, consoleFontSize)
    else
    ...
    if speaktype.npcChat and
        ...
        if #highlightData > 0 then
            local colorizedText = ''
            local lastIndex = 1
            local highlightColor = '#1f9ffe'
            local currentLen = 0

            for i = 1, #highlightData / 3 do
                local startIdx = highlightData[(i - 1) * 3 + 1]
                local endIdx = highlightData[(i - 1) * 3 + 2]
                local word = highlightData[(i - 1) * 3 + 3]

                if startIdx > lastIndex then
                    local segment = text:sub(lastIndex, startIdx - 1)
                    colorizedText = colorizedText .. string.format('{%s, %s}', segment, speaktype.color)
                    currentLen = currentLen + #segment
                end

                colorizedText = colorizedText .. string.format('{%s, %s}', word, highlightColor)
                for k = 1, #word do
                    label.highlightInfo[currentLen + k] = word
                end
            ...
            if lastIndex <= #text then
                local segment = text:sub(lastIndex)
                colorizedText = colorizedText .. string.format('{%s, %s}', segment, speaktype.color)
            end

            label:setColoredText(colorizedText)
            processedText = text:gsub('%{(.-)%}', '%1')
            isHighlighted = true
        end
    end
    ...
    if readOnlyModeEnabled and activeactiveReadOnlyTabName == tab:getText() then
        ...
        if consoleFontSize > 0 then
            readOnlyLabel:setTTFFont(consoleFont, consoleFontSize)
        else
            readOnlyLabel:setFont(consoleFont)
        end
