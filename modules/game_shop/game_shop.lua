...
function buyConfirmed()
  local protocolGame = g_game.getProtocolGame()
  if protocolGame then
    protocolGame:sendExtendedOpcode(GAME_SHOP_CODE, json.encode({ action = "purchase", data = {
...
function confirmGiftCoins()
  ...
  local protocolGame = g_game.getProtocolGame()
  if protocolGame then
    protocolGame:sendExtendedOpcode(GAME_SHOP_CODE, json.encode({ action = "transfer", data = { 
